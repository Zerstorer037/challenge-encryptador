...
function desencriptar(palabraEncriptada) {
    ...
    for (let i = 0; i < letrasEncriptadas.length; i++) {
        palabraDesencriptada = palabraDesencriptada.split(letrasEncriptadas[i]).join(letrasOriginales[i]);
    }
...
function mostrarResultado(resultado) {
    ...
    const mensajesMostrar = document.querySelector(".presentacion-mensajes-mostrar");
    const textoImportante = mensajesMostrar.querySelector(".texto-importante-mostrar");
    const textoNoImportante = mensajesMostrar.querySelector(".texto-no-importante-mostrar");
...
function copiarTexto() {
    const textoACopiar = document.querySelector(".texto-importante-mostrar").textContent;
